...
const Liquidations: React.FC = (props) => {
	const { data: fundings, isLoading } = usePrivateQuery<any[]>('/v1/liquidations');
		...
		return [
				...
				render(value, record, index) {
					...
					return (
						<div className=" orderly-flex orderly-items-center">
							<NetworkImage type="symbol" symbol={symbol} size="small" rounded />
			...
			},
			{
				...
				render(value, record, index) {
					...
					return <div className="orderly-text-base-contrast-98">{transfer_price}</div>;
				},
			},
			{
				title: 'Total value',
				...
				render(value, record, index) {
					...
					return (
						<div className="orderly-text-base-contrast-98">
							...
							<Text rule="symbol" symbolElement="quote" className="orderly-text-base-contrast-36 orderly-ml-[4px]">
								{symbol}
							</Text>
						</div>
					);
...

			{
				...
				render(value, record, index) {
					...
					const totalLoss = record.transfer_amount_to_insurance_fund + liquidator_fee;
