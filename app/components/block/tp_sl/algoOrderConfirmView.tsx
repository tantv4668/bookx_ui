...
export const AlgoOrderConfirmView: FC<{
	...
	const type = useMemo(() => {
		if (order.algo_type === AlgoOrderRootType.POSITIONAL_TP_SL) {
			if (typeof tp_trigger_price !== 'undefined' && typeof sl_trigger_price !== 'undefined') {
	...
	return (
					...
					{!!tp_trigger_price && (
						<div className="orderly-flex orderly-justify-between orderly-mb-1">
							...
							<Numeral
								...
								rule="price"
								coloring
								surfix={<span className="orderly-text-base-contrast-36">{symbolConfig.quote}</span>}
							/>
						</div>
					)}
					{!!sl_trigger_price && (
        ...
        <div className="orderly-h-[20px]"></div>
			...
			>
				{!props.isEditing ? (
					<div className="orderly-flex orderly-items-center orderly-h-[40px]">{/* <OrderConfirmCheckBox /> */}</div>
				) : null}
