...
export interface TextProps extends HTMLAttributes<HTMLSpanElement>, VariantProps<typeof textVariants> {
	asChildren?: boolean;
	rule?: TextRule;
...
export const Text: FC<PropsWithChildren<TextProps>> = (props) => {
	const { variant, rule, asChildren, type, className, children, symbolElement, surfix, formatString, ...rest } = props;
	...
	const content = useMemo(() => {
		...
		if (rule === 'date') {
			...
			return dayjs(children as string).format(formatString ?? 'YYYY-MM-DD HH:mm:ss');
		}
		if (rule === 'symbol') {
	...
	return <Comp {...rest} className={cn(textVariants({ variant, type, className }))} children={contentWithSurfix} />;
};
