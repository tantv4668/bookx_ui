...
const HoverCard: React.FC<HoverCardProps & React.ComponentPropsWithoutRef<typeof HoverCardPrimitive.Content>> = (
	props,
) => {
	const { content, open, defaultOpen, onOpenChange, children, ...rest } = props;
	return (
		<HoverCardRoot open={open} defaultOpen={defaultOpen} onOpenChange={onOpenChange}>
			<HoverCardTrigger asChild>{children}</HoverCardTrigger>
			<HoverCardPrimitive.Portal>
				<HoverCardContent {...rest}>
					{content}
					<HoverCardPrimitive.Arrow width={11} height={5} className="orderly-fill-base-400" />
				</HoverCardContent>
			</HoverCardPrimitive.Portal>
		</HoverCardRoot>
	);
