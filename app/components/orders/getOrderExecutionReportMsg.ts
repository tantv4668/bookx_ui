...
export function getOrderExecutionReportMsg(data: API.AlgoOrder | API.Order, symbolsInfo: any) {
	...
	switch (status) {
		...
		case 'PARTIAL_FILLED':
			const displayTotalExecutedQuantity =
				base_dp === undefined ? total_executed_quantity : parseNumber(total_executed_quantity, { precision: base_dp });
			title = 'Order filled';
