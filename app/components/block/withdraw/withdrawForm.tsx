...
export const WithdrawForm: FC<WithdrawProps> = ({
	...
	const [inputStatus, setInputStatus] = useState<any>('default');
	...
	const openChainPicker = useCallback(async () => {
		const result = await modal.show<{ id: number; name: string }, any>(ChainDialog, {
			testChains: chains,
	...
	return (
		<div id="orderly-withdraw-form">
			...
			<div className="orderly-flex orderly-items-center orderly-text-2xs desktop:orderly-text-base">
				...
				<NetworkImage type={typeof walletName === 'undefined' ? 'placeholder' : 'wallet'} name={walletName} rounded />
			</div>
			<div className="orderly-py-2">
				<WalletPicker address={address} chain={chain} onChainChange={onChainChange} />
			</div>
			<TokenQtyInput
				amount={quantity}
				fee={fee}
