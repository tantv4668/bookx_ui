...
export const WalletConnect: FC<WalletConnectProps> = (props) => {
	...
	const [bindRefCode, { error: updateOrderError, isMutating: updateMutating }] = useMutation(
		'/v1/referral/bind',
		'POST',
	);
	...
	return (
		<div id="orderly-wallet-connect-view">
			...
			<Paper className="orderly-bg-base-500">
				{!isExpired && (
					<ListTile
						...
						title="Sign in"
						disabled={status < AccountStatusEnum.NotConnected || status >= AccountStatusEnum.SignedIn}
			...
			{(referral_code?.length || 0) === 0 && loadingReferralCode === false && (
				<ReferralCode className="orderly-pt-5" refCode={refCode} setRefCode={setRefCode} isExist={isExist} />
			)}

			<div className="orderly-py-4 orderly-flex orderly-justify-between orderly-items-center">
				<RememberMe />
				<div>
					<Switch id="orderly-remember-me-switch" checked={remember} onCheckedChange={setRemember} />
				</div>
			</div>
			<div>
				<Button
