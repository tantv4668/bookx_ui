...
export const TPSLForm: FC<Props> = (props) => {
	...
	const dirty = useMemo(() => {
		const quantity =
			props.oldOrder?.algo_type === AlgoOrderRootType.POSITIONAL_TP_SL ? maxQtyNumber : props.oldOrder?.quantity;
		...
		if (props.oldOrder) {
			const tp = props.oldOrder.child_orders.find((o) => o.algo_type === AlgoOrderType.TAKE_PROFIT);
			const sl = props.oldOrder.child_orders.find((o) => o.algo_type === AlgoOrderType.STOP_LOSS);
			...
			if (sl?.trigger_price !== Number(order.sl_trigger_price) && typeof order.sl_trigger_price !== 'undefined') {
				diff = 3;
			}
		}
		...
		return diff;
	}, [order.tp_trigger_price, order.sl_trigger_price, order.quantity, props.oldOrder]);
	...
	return (
		<div id="orderly-tp_sl-form" className={cn('orderly-space-y-4 orderly-text-3xs', props.className)}>
					...
					<div>
						<div className={'orderly-flex'}>
							<Input
								...
								value={isPositionTPSLOrder && !props.isEditing ? '' : commify(props.order.quantity ?? 0)}
								className={'orderly-text-right'}
								containerClassName={'desktop:orderly-bg-base-700 orderly-bg-base-500 orderly-flex-1'}
								name="orderQuantity"
								...
								// }}
								onValueChange={(value) => {
									props.onChange('quantity', todpIfNeed(value, symbolInfo('base_dp')));
								}}
							/>
							{!props.isEditing && (
								<button
									...
									className={cn(
										...
										isPositionTPSLOrder && 'orderly-border-primary orderly-text-primary',
									)}
								>
									Position
								</button>
						...
						/>
						<div className={'orderly-flex orderly-justify-between orderly-text-primary orderly-text-2xs'}>
							<span>0%</span>
							<button
							...
							>
								<span>Max</span>
								<Numeral className={'orderly-text-base-contrast-54 orderly-ml-1'} precision={symbolInfo('base_dp')}>
									{maxQty}
								</Numeral>
							</button>
						</div>
			...
			<div>
				<div className={'orderly-flex orderly-justify-between orderly-mb-[8px] orderly-items-center'}>
					<div className="orderly-text-xs">Take profit</div>
					<div className={'orderly-text-base-contrast-36 orderly-text-2xs'} data-testid="tpEstPnL">
						<span>Est. PnL: </span>
						{order.tp_pnl ? (
				...
				<div className={'orderly-grid orderly-grid-cols-2 orderly-gap-2'}>
					<Input
						...
						className={'orderly-text-right orderly-pr-2 orderly-text-3xs placeholder:orderly-text-3xs'}
			...
			<div>
				<div className={'orderly-flex orderly-justify-between orderly-mb-[8px] orderly-items-center'}>
					...
					<div className={'orderly-text-base-contrast-36 orderly-text-2xs'} data-testid="slEstPnL">
						<span>Est. PnL: </span>
						{order.sl_pnl ? (
							<Numeral rule="price" coloring precision={symbolInfo('quote_dp')}>
								{order.sl_pnl}
			...
			<div className={'orderly-flex orderly-justify-center orderly-gap-3 orderly-pt-2 desktop:orderly-pt-0'}>
				<Button
					color={'tertiary'}
					className={'orderly-flex-1 desktop:orderly-w-[98px] desktop:orderly-h-[32px] desktop:orderly-flex-none'}
