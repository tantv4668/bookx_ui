...
export const MSelect: FC<SelectProps> = ({ className, size, disabled, color, fullWidth, ...props }) => {
	...
	useEffect(() => {
		const onTouchStart = (event: TouchEvent) => {
			if (containerRef.current === event.target || containerRef.current?.contains(event.target as Element)) {
				return;
			}
			setOpen(false);
...
export const SelectMenuItem: FC<PropsWithChildren<SelectMenuItemProps>> = (props) => {
