...
import { useCollateral, usePositionStream, useMarginRatio, useLeverage } from '@orderly.network/hooks';
...
import { StatisticStyleProvider } from '@/app/components/statistic/defaultStaticStyle';
import { Statistic } from '@/app/components/statistic';
import { Numeral } from '@/app/components/text';
import { Divider } from '@/app/components/divider';
import { Paper } from '@/app/components/layout';
...
export const AssetAndMarginSheet: FC<AssetAndMarginProps> = (props) => {
	const { totalCollateral, freeCollateral, totalValue, availableBalance } = useCollateral({
		dp: 2,
	});
	const [{ aggregated, totalUnrealizedROI }, positionsInfo] = usePositionStream();
	const { marginRatio, currentLeverage, mmr } = useMarginRatio();
	const { visible, toggleVisible, onDeposit, onWithdraw } = useContext(AssetsContext);
		...
		return modal.confirm({
			...
			content: (
				<div className="orderly-text-base-contrast-54 orderly-text-2xs desktop:orderly-text-sm">
					Are you sure you want to settle your PnL? Settlement will take up to 1 minute before you can withdraw your
					available balance.
				</div>
			),
			onCancel: () => {
					...
					<Statistic
						...
						value={
							<div className="orderly-flex orderly-justify-between">
								<Numeral rule="price" visible={visible} coloring className="orderly-text-2xs">
