...
import { useOrderbookStream, useSymbolsInfo, useEventEmitter } from '@orderly.network/hooks';
...
export const MyOrderBook: FC<MyOrderBookProps> = (props) => {
	...
	useEffect(() => {
		if (height?.content) {
			...
			const level = Math.floor((height.content - SPACE) / ((DEFAULT_CELL_HEIGHT + 1) * 2));
