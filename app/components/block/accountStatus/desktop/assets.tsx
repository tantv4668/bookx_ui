...
import { MemorizedLeverage } from './leverage';
...
export const Assets: FC<AssetsProps> = (props) => {
	...
	const [getTestUSDC, { isMutating }] = useMutation(`${config.get('operatorUrl')}/v1/faucet/usdc`);
	...
	return (
		...
		>
			<div className={'orderly-py-4 orderly-flex orderly-justify-between orderly-items-center orderly-tabular-nums'}>
				<div className={'orderly-flex-1'}>
					<div className={'orderly-text-3xs orderly-text-base-contrast-54'}>Total value</div>
					<div>
						<Numeral
				...
				<CollapsibleTrigger asChild>
					<button className="orderly-p-1 orderly-rounded hover:orderly-bg-base-900 data-[state=open]:orderly-rotate-180 orderly-transition-transform">
						...
						<ChevronDown size={18} className={'orderly-text-base-contrast-54 orderly-z-0'} />
					</button>
				</CollapsibleTrigger>
			</div>
			{showGetTestUSDC && (
			...
			<MemorizedLeverage isConnected={isConnected} data-margin-ratio={isConnected ? marginRatioAttr : undefined} />
		</Collapsible>
	);
};
