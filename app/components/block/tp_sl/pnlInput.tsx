...
export const PnlInput: FC<Props> = (props) => {
	...
	const [mode, setMode] = useLocalStorage<PnLMode>('TP/SL_Mode', PnLMode.PERCENTAGE);
	...
	const value = useMemo(() => {
		...
		if (mode === PnLMode.PERCENTAGE) {
			return `${new Decimal(val).mul(100).todp(2, 4).toString()}${percentageSuffix.current}`;
	...
	return (
		<Input
			...
			className={cn(
				...
				pnlNumber > 0 ? 'orderly-text-trade-profit' : pnlNumber === 0 ? '' : 'orderly-text-trade-loss',
			)}
			...
			suffix={
				<DropdownMenu modal={false}>
					<DropdownMenuTrigger asChild>
						<button
							className={'orderly-px-2 orderly-h-full orderly-group active:orderly-outline-0'}
					...
					>
						<DropdownMenuItem onSelect={() => setMode(PnLMode.PnL)} data-testid={`${PnLMode.PnL}_menu_item`}>
							{PnLMode.PnL}
						</DropdownMenuItem>
						<DropdownMenuItem onSelect={() => setMode(PnLMode.OFFSET)} data-testid={`${PnLMode.OFFSET}_mneu_item`}>
							{PnLMode.OFFSET}
						</DropdownMenuItem>
						<DropdownMenuItem
