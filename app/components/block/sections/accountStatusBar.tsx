...
export type AccountStatus = 'NotConnected' | 'Connected' | 'NotSignedIn' | 'EnabledTrading' | 'SignedIn';
...
export const AccountStatusBar: FC<AccountStatusProps> = (props) => {
	...
	return (
		<div id="orderly-bottom-bar" className="orderly-flex orderly-items-center orderly-justify-between orderly-w-full">
			{status !== AccountStatusEnum.NotConnected && !errors?.ChainNetworkNotSupport ? (
				<AccountTotal
					status={status}
				...
				{status === AccountStatusEnum.NotConnected ? (
					...
					<Sheet open={infoOpen} onOpenChange={setInfoOpen}>
						...
						<SheetContent id="my-account" forceMount>
							<SheetHeader id="my-account-sheet-title" leading={<Logo.secondary size={30} />}>
								My account
							</SheetHeader>
							<AccountInfo
