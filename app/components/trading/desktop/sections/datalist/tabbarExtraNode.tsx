...
export const TabBarExtraNode: FC<TabBarExtraNodeProps> = (props) => {
	const [showAllSymbol, setShowAllSymbol] = useSessionStorage('showAllSymbol_position', true);
	...
	return (
		<div className={'orderly-flex orderly-items-center orderly-gap-2'}>
			<div className={'orderly-flex orderly-items-center orderly-gap-2'}>
				<Checkbox id={'showAll'} checked={showAllSymbol} onCheckedChange={onShowAllSymbolChange} />
				<Label htmlFor={'showAll'} className="orderly-text-base-contrast-54 orderly-text-3xs">
...
const SettingIcon: FC<TabBarExtraNodeProps> = (props) => {
	...
	const onDecimalPrecisionChanged = useCallback(
		(type: DecimalPrecisionType) => {
			var value: number = 0;
			switch (type) {
	...
	const onUnPnlPriceBasisChanged = useCallback(
		(type: UnPnlPriceBasisType) => {
			var value: number = 0;
			switch (type) {
	...
	return (
		<DropdownMenu open={open} onOpenChange={setOpen}>
			<DropdownMenuTrigger>
				<button
					className={
						'orderly-pb-[1px] orderly-h-[48px] orderly-text-base-contrast-80 orderly-cursor-pointer orderly-fill-base-contrast-54 hover:orderly-fill-base-contrast'
					}
				>
					{/* <Settings size={18} /> */}
			...
			>
				<div className="orderly-flex orderly-flex-col orderly-text-3xs">
					...
					<Divider />
					<span className="orderly-pb-3 orderly-text-base-contrast-54 orderly-mt-2">
