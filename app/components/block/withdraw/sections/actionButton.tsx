...
export const ActionButton: FC<ActionButtonProps> = (props) => {
	...
	const onClick = useCallback(() => {
		...
		if (crossChainWithdraw) {
			modal.confirm({
				...
				content: <CrossChainConfirm address={address || ''} amount={qty - fee} chain={chain} />,
				onOk: async () => {
					onWithdraw();
				},
				onCancel: async () => {},
	...
	}, [chainNotSupport, chains, switchChain, disabled, chain, quantity, loading]);
	...
	return (
		<>
			...
			<div className="orderly-flex orderly-justify-center orderly-text-xs desktop:orderly-text-xs orderly-font-bold">
				<div className="orderly-withdraw-action-button-container orderly-w-full">{actionButton}</div>
			</div>
		</>
	);
};
