...
import { useOrderStream, useAccount, useSessionStorage } from '@orderly.network/hooks';
// import { TradingPageContext } from "@/page/trading/context/tradingPageContext";
import { API, AccountStatusEnum, AlgoOrderRootType, OrderEntity } from '@orderly.network/types';
...
export const MyOrders: FC<Props> = (props) => {
	...
	const [data, { isLoading, loadMore, cancelOrder, updateOrder, cancelAlgoOrder, updateAlgoOrder }] = useOrderStream({
		status: props.status,
		symbol: tabExtraData.showAllSymbol ? '' : context.symbol,
		// @ts-ignore
		side,
		excludes: props.status === OrderStatus.FILLED ? [] : [AlgoOrderRootType.POSITIONAL_TP_SL, AlgoOrderRootType.TP_SL],
	});
	...
	const onCancelOrder = useCallback((orderId: number, symbol: string): Promise<any> => {
