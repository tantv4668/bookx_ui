...
const MenuItem = (props: { icon: ReactNode; title: string; className?: string; onClick: () => void; id?: string }) => (
	<div
		id={props.id}
		className={cn(
	...
	>
		{props.icon}
		<span className="orderly-text-xs orderly-pl-2 orderly-font-semibold">{props.title}</span>
	</div>
);
